...
const CartSubmit = () => {
    ...
    function submitOrder(event) {
        ...
        Array.from(cartItems.entries()).forEach(([key, value1]) => {
            if (value1 === 1) {
                itemsInOrder.push(key);
            } else if (value1 > 1) {
                    ...
                    <div className={"user-info-cart"}>
                        <div className={"user-form"}>
                            <form className={"form-user"} onSubmit={submitOrder}>
                                ...
                                <input disabled={true} value={userInfo.surname} name="surname" id="surname"
                                       placeholder="Прізвище"/>
                                <label htmlFor="email">Електронна пошта</label>
                                <input type="email" disabled={true} value={userInfo.email}
                                ...
                                <input
                                    ...
                                    maxLength="13"/>
                                {cartItems.size === 0 ?
                                    <button disabled={true} className={"leaveRequest-button"}>Замовити</button> :
                                    <button className={"leaveRequest-button"}>Замовити</button>}
                            </form>

                        </div>
                    </div>
                </div>) : (<Payment summ={totalSum} data={cartConfig}/>)}
